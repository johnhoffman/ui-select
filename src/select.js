(function () {
  ...
  .service('ScrollParser', ['uiSelectMinErr','$parse', function(uiSelectMinErr, $parse) {
    ...
    /**
     * Example:
     * expression = "address in addresses | filter: {street: $select.search}"
     * itemName = "address",
     * source = "addresses | filter: {street: $select.search}",
     */
    self.parse = function(expression) {

      var match = expression.match(/^\s*([\S]+?)\s+in\s+([\s\S]+?)\s*$/);

      if (!match) {
        throw uiSelectMinErr('iexp', "Expected expression in form of '_item_ in _collection_' but got '{0}'.",
                expression);
      }

      return {
        itemName: match[1], // (lhs) Left-hand side,
        source: $parse(match[2]),
        modelMapper: $parse(match[1])
      };

    };

    self.getUiScrollExpression = function(itemName, source) {
      return itemName + ' in ' + source;
    };
  }])
